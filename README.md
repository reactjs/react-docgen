...
| resolver     | function | A function of the form `(ast: ASTNode, recast: Object) => (NodePath\|Array<NodePath>)`. Given an AST and a reference to recast, it returns an (array of) NodePath which represents the component definition. Built-in Resolvers are available under `resolver` property, e.g. `reactDocgen.resolver.findAllComponentDefinitions`|
